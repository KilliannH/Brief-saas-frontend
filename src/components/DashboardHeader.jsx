...
export default function DashboardHeader({ filter, onFilterChange, isFreeLimited, onNewBriefClick, totalPages, currentPage, onPageChange }) {
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center gap-2">
        {isFreeLimited ? (
          <>
            ...
            <span className="text-sm text-gray-500">{t("dashboard.restriction.limitReached")}</span>
            <Link to="/account" className="text-blue-600 text-sm underline hover:text-blue-800">
        ...
        <div className="flex justify-center md:justify-end mt-4 md:mt-0 space-x-2">
          ...
          {[...Array(totalPages)].map((_, i) => (
            <button
              ...
              className={`px-3 py-1 rounded ${i === currentPage ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-700"}`}
            >
              {i + 1}
            </button>
          ))}
